...
function showlist(data) {
  ...
  if (str.length > 0) {
    str.forEach(function(item, index) {
      ...
      if (url == undefined) url = item.meta_pages[0].image_urls.original;
      ...
      elem =
        ...
        '" alt ="' +
        item.title +
        '"  class="image" rel="https://www.pixiv.net/member_illust.php?mode=medium&illust_id=' +
        item.id +
        '">\
				        <img src ="https://img.pixivic.com/get/' +
        item.image_urls.large.replace("_webp", "") +
        '" height ="' +
        imgH +
        '"  width="228" alt="' +
        item.title +
...
function showtags(data) {
  ...
  for (var i = 0, len = data.length; i < len; i++) {
    str +=
      '<div class="tag"><div class="title" onclick="research(this)">' +
      data[i].tag +
      '</div><div class="des">' +
      data[i].tag_translation +
      "</div></div>";
  }
